...
1.  Since we use the DenyOverrides algorithm, the order of sub-PolicySets is not important; all will be evaluated and any `Deny` will override any `Allow`. The `PolicySetId` has to be a new UUID for each new document.
2.  A mandatory but not-quite-useful description.
3.  The patient EPR-SPID.

## Emergency access

Healthcare professionals that are not specifically granted access still can obtain a temporary access to the patient record (i.e. break the glass). Emergency access can be granted or denied, its scope is all healthcare professionals registered.
...
1.  Don't forget to assign a random value to the `@PolicySetId`.
...
The eMedication service does a bit of magic with the APPC document when it comes to evaluate its rules. Some specific rules are added. These roles cannot be targeted by custom rules, their access rights are granted by the eMedication service.

1.  The patient themself has full (normal) access to their record, ever.
2.  Document administrators have (normal) access to all eMedication documents but not to administrative (APPC) documents.
3.  Policy administrators have (normal) access to all administrative documents but not to eMedication documents.
4.  Technical users have a write-only (auto) access for eMedication documents.
